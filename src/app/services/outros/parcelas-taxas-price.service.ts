...
export class ParcelasTaxasPriceService {
   ...
   * @return parcelaArray
   */    
  async calculaParcelas(finan: FinanImovelPrice) {
    let parcelas: any[] = [];
    ...
    for (var i = 0; i < (finan.qtd_parcelas_meses + 1); i++) {
      ...
      if (i > 0) {  
        const mesVsParcela = Math.pow(1 + finan.percent_juros_mensal, finan.qtd_parcelas_meses);
        parcela.valor       = finan.saldo_devedor_val * ((mesVsParcela * finan.percent_juros_mensal) / ( mesVsParcela -1));
        parcela.juros       = saldoDevedor ? saldoDevedor * finan.percent_juros_mensal : null;
        parcela.amortizacao = parcela.valor - parcela.juros;
        saldoDevedor  = saldoDevedor ? saldoDevedor - parcela.amortizacao : null;
        ...
        saldoDevedor = finan.saldo_devedor_val;
        parcela.saldoDevedor = this.helper.arredondarPraCima(saldoDevedor, 0);
   ...
   * @return parcela
   */     
  async taxasPorParcela(parcela: any, finan: FinanImovelPrice) {
    parcela.somaTaxas = 0;
    for (var i = 1; i < 6; i++) { 
      ...
      if (finan[fieldLabel]) {
        ...
        if (parcela.numParcela === 0 && !finan[fieldCheck]){
          parcela[fieldVal] = 0;
        }
/*         if (parcela.numParcela === finan.qtd_parcelas_meses && finan[fieldCheck] === true){
   ...
   * @return finan
   */    
  async taxasTotaisFinam(finan: FinanImovelPrice, parcela: any) {
    for (var i = 1; i < 6; i++) {
      const fieldLabel    = 'taxa' + i + '_mensal';
      const fieldVal      = 'taxa' + i + '_mensal_val';
